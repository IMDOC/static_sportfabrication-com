...
def parse_article(path: Path) -> dict | None:
    ...
    tags = raw_tags
    read_time = m.get('twitter:data1') or '5 min de lecture'
...
def render_card(art: dict) -> str:
    ...
    date = art.get('date', '')
    read_time = html_escape(art.get('readTime', '5 min de lecture'))
    url = art.get('url', '#')
  ...
  <div class="blog-card-body">
    ...
    <a href="{url}" class="blog-card-read-more">Lire la suite <i class="bi bi-arrow-right"></i></a>
  </div>
</article>'''
...
def ssr_blog_list(root: Path, articles: list[dict], categories_manifest: dict, authors_manifest: dict, tags_manifest: dict) -> bool:
    ...
    cards = '\n'.join(render_card(a) for a in articles) if articles else \
            '<p class="empty-state">De nouveaux articles arrivent bientôt.</p>'
...
def render_sidebar(current_type: str, current_slug: str, categories: list[dict], tags: list[dict], authors: list[dict]) -> str:
    ...
    return f'''<aside class="blog-sidebar">
      <div class="sidebar-widget">
        <h3 class="sidebar-title"><i class="bi bi-folder2-open"></i> Catégories</h3>
        <ul class="sidebar-list sidebar-categories">{cat_html}</ul>
      </div>
      <div class="sidebar-widget">
        <h3 class="sidebar-title"><i class="bi bi-tags"></i> Tags populaires</h3>
        <div class="sidebar-tags">{tag_html}</div>
      </div>
      <div class="sidebar-widget">
        <h3 class="sidebar-title"><i class="bi bi-people"></i> Auteurs</h3>
...
<head>
    ...
    {{
      ...
      "@type": "BreadcrumbList",
      "itemListElement": [
        {{ "@type": "ListItem", "position": 1, "name": "Accueil", "item": "https://www.{domain}/" }},
    ...
    <div class="container">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-9 order-lg-2 order-1">
                <h2 class="articles-heading">Articles dans {name}</h2>
                <div class="row g-4">
                    <!-- CATEGORY_GRID_START -->
                    <!-- CATEGORY_GRID_END -->
                </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{tag_title} — Articles tagués {tag_display} | {brand}</title>
    ...
    <meta property="og:type" content="website">
    <meta property="og:title" content="{tag_title} — Articles tagués {tag_display}">
    ...
    {{
      "@context": "https://schema.org",
      "@type": "CollectionPage",
      "name": "{tag_title} — Articles tagués {tag_display}",
...
<section class="page-section-wrap">
    <div class="container">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-9 order-lg-2 order-1">
                <h2 class="articles-heading">Articles tagués {tag_display}</h2>
                <div class="row g-4">
                    <!-- TAG_GRID_START -->
                    <!-- TAG_GRID_END -->
                </div>
...
def build_tag_pages(root: Path, articles: list[dict], tags_manifest: dict, categories_manifest: dict, authors_manifest: dict, brand: str, domain: str, topbar_config: str) -> int:
    ...
    for tag_obj in tags_manifest.get('tags', []):
        slug = tag_obj['slug']
        name = tag_obj['name']
        desc = tag_obj.get('description', f'Articles tagués {name}.')
        arts = by_slug.get(slug, [])
        ...
        cards = '\n'.join(render_card(a) for a in arts) if arts else \
                f'<p class="empty-state">Aucun article tagué <strong>{html_escape(name)}</strong>.</p>'
...
def ssr_author_pages(root: Path, articles: list[dict], authors_manifest: dict) -> int:
    ...
    for author in authors_manifest.get('authors', []):
        ...
        cards = '\n'.join(render_card(a) for a in arts) if arts else \
                f'<p class="empty-state">Nouveaux articles de {author["name"]}.</p>'
        html = target.read_text(encoding='utf-8')
...
def write_sitemap(root: Path, domain: str) -> int:
    ...
    # Sub-folders: blog/, category/, tag/, author/ + landing subdirs (produits/, services/)
    for sub in ('blog', 'category', 'tag', 'author', 'produits', 'services'):
        sub_dir = root / sub
        if sub_dir.is_dir():
